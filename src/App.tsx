...
const AppContent: React.FC = () => {
  ...
  const [reloadKey, setReloadKey] = useState(0); // Forçar reload do gerenciar
  ...
  const voltarParaGerenciar = () => {
    ...
    // Forçar reload do GerenciarFormularios
    setReloadKey(prev => prev + 1);
    console.log('🔄 Voltando para gerenciar - forçando reload...');
  };
    ...
    if (telaAtiva === 'GERENCIAR') {
      return (
        <GerenciarFormularios
          key={reloadKey} // Força remontação do componente
      ...
      <ProtectedRoute>
          <div style={{ 
            ...
            <NavegacaoFormularios 
              telaAtiva={telaAtiva}
              onMudarTela={(tela) => {
                // Só limpar formulário se não estiver no modo edição ou se for para gerenciar
                if (!formularioEditando || tela === 'GERENCIAR') {
                  setFormularioEditando(null);
                }
                setTelaAtiva(tela);
              }}
